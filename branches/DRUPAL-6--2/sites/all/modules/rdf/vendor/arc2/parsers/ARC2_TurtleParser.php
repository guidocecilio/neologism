...
version:  2009-02-09 (Tweak: Leading [\xA0\xC2] is getting auto-removed in method x()
                      )
*/
...
class ARC2_TurtleParser extends ARC2_RDFParser {
  ...
  function xString($v) {/* largely simplified, may need some tweaks in following revisions */
    ...
    if (preg_match('/^([^\x5c]*|.*[^\x5c]|.*\x5c{2})' . str_replace("'", "\'", $delim) . '(.*)$/sU', $rest, $m)) {
      return array(array('value' => $this->toUTF8($m[1]) , 'type' => 'literal', 'sub_type' => $sub_type), $m[2]);
    }
    return array(0, $v);
  }
